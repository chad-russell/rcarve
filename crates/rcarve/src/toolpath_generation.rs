...
fn generate_toolpath_for_operation(
    ...
    match operation {
        Operation::Profile {
            ...
            let polygon = first_polygon(shapes, targets)?.context("Profile requires geometry")?;
        ...
        Operation::Pocket {
            ...
            let (outer, holes) = match target {
                OperationTarget::Region(region_id) => {
                    ...
                    flatten_region(shapes, region)?
                }
                OperationTarget::Curves(curves) => {
                    let mut polys = flatten_curves(shapes, curves)?;
                    let outer = polys
                        .pop()
        ...
        Operation::VCarve {
            ...
            let carve_polygons = collect_vcarve_polygons(shapes, targets)?;
...
pub fn polygons_for_operation(project: &Project, index: usize) -> Result<Vec<Vec<(f64, f64)>>> {
    ...
    match operation {
        Operation::Profile { targets, .. } => {
            let polygon = first_polygon(shapes, targets)?;
        ...
        Operation::VCarve { targets, .. } => match targets {
            OperationTarget::Curves(curves) => flatten_curves(shapes, curves),
            OperationTarget::Region(region_id) => {
                let region = shapes
                    .get_region(region_id)
                    .ok_or_else(|| anyhow!("Region {:?} not found", region_id))?;
                let (mut outer, mut holes) = flatten_region(shapes, region)?;
                close_loop(&mut outer);
                for hole in &mut holes {
                    close_loop(hole);
                }
    ...
    match target {
        OperationTarget::Curves(curves) => {
            if let Some(first) = curves.first() {
                let mut points = flatten_curve(shapes, first)?;
        ...
        OperationTarget::Region(region_id) => {
            let region = shapes
                .get_region(region_id)
                .ok_or_else(|| anyhow!("Region {:?} not found", region_id))?;
            let (mut outer, _) = flatten_region(shapes, region)?;
            close_loop(&mut outer);
            Ok(Some(outer))
        }
    }
...
fn flatten_curves(
    ...
    for id in curves {
        let mut points = flatten_curve(shapes, id)?;
        close_loop(&mut points);
        if points.len() >= 3 {
            result.push(points);
        }
...
fn flatten_region(
    ...
    let mut outer = flatten_curve(shapes, &region.outer)?;
    close_loop(&mut outer);

    let mut holes = Vec::new();
    for hole in &region.holes {
        let mut points = flatten_curve(shapes, hole)?;
        close_loop(&mut points);
        if points.len() >= 3 {
            holes.push(points);
        }
...
fn collect_vcarve_polygons(
    ...
    match targets {
        OperationTarget::Curves(curves) => {
            let loops = flatten_curves(shapes, curves)?;
            for outer in loops {
        ...
        OperationTarget::Region(region_id) => {
            let region = shapes
                .get_region(region_id)
                .ok_or_else(|| anyhow!("Region {:?} not found", region_id))?;
            let (outer, holes) = flatten_region(shapes, region)?;
...
fn flatten_curve(shapes: &crate::geometry::ShapeRegistry, id: &CurveId) -> Result<Vec<(f64, f64)>> {
    let curve = shapes
        .get_curve(id)
